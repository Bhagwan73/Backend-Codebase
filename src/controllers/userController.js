  ...
  try {
    ...
    if (!titles.includes(title))
      return res.status(400).send({
        status: false,
        message: "please provide the valid title Mr or Mrs or Miss ",
      });
    ...
    if (!isvalidMobile(phone))
      return res.status(400).send({
        status: false,
        message: "plese provide the valid phone number",
      });
...
const userLogin = async function (req, res) {
  try {
    ...
    if (!emailId)
      return res.status(400).send({
        status: false,
        message: "your account not found please create account first",
      });
    ...
    let getUsersData = await userModel.findOne({
      email: email,
      password: password,
    });
    if (!getUsersData)
